...
// ADD FLOOR
const floorGeo = new THREE.PlaneBufferGeometry(5.0, 5.0);
const floor = new THREE.Mesh(floorGeo, matNormal);
floor.position.set(0.25, -0.5, -0.25);
...
cylinderGeometry = new THREE.CylinderGeometry( 0.10, 0.2, 0.80, 20, 4 );
cylinder = new THREE.Mesh( cylinderGeometry, matNormal);
...
// parameters:    
//    radiusAtTop, radiusAtBottom, height, segmentsAroundRadius, segmentsAlongHeight, segmentsAlongHeight
coneGeometry = new THREE.CylinderGeometry( 0.0, 0.80, 1, 20, 4 );
cone1 = new THREE.Mesh( coneGeometry, matNormal);
cone1.position.set(0.0, 0.8, 0.0);
scene.add( cone1);

coneGeometry = new THREE.CylinderGeometry( 0.0, 0.60, 1, 20, 4 );
cone2 = new THREE.Mesh( coneGeometry, matNormal);
cone2.position.set(0.0, 1.3, 0.0);
scene.add( cone2);

coneGeometry = new THREE.CylinderGeometry( 0.0, 0.45, 0.80, 20, 4 );
cone3 = new THREE.Mesh( coneGeometry, matNormal);
cone3.position.set(0.0, 1.75, 0.0);
scene.add( cone3);
...
directionalLight.position.set( -1, 2.0, 1.0 );
...
sphereGeometry = new THREE.SphereGeometry(0.1, 15, 15);    // radius, segments, segments
lightSphere = new THREE.Mesh(sphereGeometry, basicMaterial);
lightSphere.position.set(0,4,-5);
lightSphere.position.set(directionalLight.position.x, directionalLight.position.y, directionalLight.position.z);
...
const FRAGMENT_FINAL = `
    ...
    void main()
    {
        ...
        vec2 uv = vUv;
        uv += vec2(triangle(uv.y * sin(time) * 1.0) * sin(time * 2.9) * 0.025,
                triangle(uv.x * sin(time) * 1.0) * sin(time * 3.1) * 0.025);
...
const finalShader = {
    uniforms: {
        ...
        tNoise: { type: 't', value: new THREE.TextureLoader().load('images/noise.png')}
    },
    vertexShader: VERTEX,
...
const render = () => {
    ...
    cylinder.material = matShadow;
    cone1.material = matShadow;
    cone2.material = matShadow;
    cone3.material = matShadow;
    
    renderer.render(scene, camera, shadowBuffer);
    pass.uniforms.tShadow.value = shadowBuffer.texture;

    floor.material = matNormal;
    cylinder.material = matNormal; 
    cone1.material = matNormal; 
    cone2.material = matNormal; 
    cone3.material = matNormal;
